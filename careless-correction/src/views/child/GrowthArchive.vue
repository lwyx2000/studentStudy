...
<template>
  <div class="page">
    <section class="page-hero"><div class="hero-card"><span class="eyebrow">📈 长期成长档案与预警中心</span><h1>把粗心变化看成发展曲线</h1><p class="lead">长期档案汇总任务完成率、漏题率、物品流失频次与错题诊断数等长期指标，只呈现可行动建议，不制造焦虑。</p></div><div class="panel"><div class="card-title"><h2>{{ growthStore.alerts[0]?.title }}</h2><span class="tag">近 7 天</span></div><p class="lead">{{ growthStore.alerts[0]?.description }} {{ growthStore.alerts[0]?.suggestion }}</p></div></section>
