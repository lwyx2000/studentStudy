...
<template>
  <div class="page printable-page">
    ...
    <section class="sheet"><header><h1>小树成长岛 · 本周打卡单</h1><p>姓名：{{ userStore.profile.name }}　年级：{{ userStore.profile.grade }} 年级　完成：{{ completionText }}</p></header><table><thead><tr><th>日期</th><th v-for="task in taskStore.todayTasks" :key="task.id">{{ task.title }}</th><th>家长只观察不催促</th></tr></thead><tbody><tr v-for="day in days" :key="day"><td>{{ day }}</td><td v-for="task in taskStore.todayTasks" :key="task.id">□</td><td>□</td></tr></tbody></table><footer>周末拍照上传，系统将扫描勾选痕迹生成成长报告。</footer></section>
  </div>
</template>
